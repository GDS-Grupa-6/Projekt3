	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float3 NormalmapXYZ148 = UnpackNormalScale( tex2D( _BumpMap, uv0_BumpMap ), _BumpScale );
				...
				float2 uv0_MainTex = float4(IN.ase_texcoord5.xy,0,0).xy * _MainTex_ST.xy + _MainTex_ST.zw;
				...
				float3 tanNormal12_g1 = NormalmapXYZ148;
				...
				float fresnelNode581 = ( 0.04 + 1.0 * pow( 1.0 - fresnelNdotV581, 1.11 ) );
				...
				#endif
				float4 lerpResult585 = lerp( ( ( _Color * tex2D( _MainTex, uv0_MainTex ) ) * float4( ( ( ( max( dotResult596 , 0.0 ) * ( ase_lightAtten * _MainLightColor.rgb ) ) + localAdditionalLightsLambert5_g1 ) + bakedGI576 ) , 0.0 ) ) , float4( indirectSpecular574 , 0.0 ) , staticSwitch240);
