Shader "Ciconia Studio/CS_Ghost/URP/CS_Fast Ghost  (XRay See Through)"
{
	Properties
	{
		...
		[Space(15)][Header(XRay Properties)][Space(15)]_XRayColor("Color -->(Transparency A)", Color) = (0,1,0.9803922,0.1019608)
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 fetchOpaqueVal15 = float4( SHADERGRAPH_SAMPLE_SCENE_COLOR( ( (ase_grabScreenPosNorm).xyzw + float4( (( mul( float4( normalizedWorldNormal , 0.0 ), UNITY_MATRIX_V ).xyz * (-1.0 + (_Refraction - 0.0) * (1.0 - -1.0) / (2.0 - 0.0)) )).xyz , 0.0 ) ).xy ), 1.0 );
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float3 ase_worldViewDir = ( _WorldSpaceCameraPos.xyz - WorldPosition );
				ase_worldViewDir = normalize(ase_worldViewDir);
				float clampResult548 = clamp( _XRayFresnelIntensity , 0.0 , 1.0 );
				float clampResult543 = clamp( _XRayFresnelPower , 0.0 , 10.0 );
				float fresnelNdotV544 = dot( ase_worldNormal, ase_worldViewDir );
				float fresnelNode544 = ( _XRayBias + clampResult548 * pow( 1.0 - fresnelNdotV544, clampResult543 ) );
				float4 lerpResult558 = lerp( float4( ( _XRayColor.a * desaturateVar278 ) , 0.0 ) , _XRayColor , fresnelNode544);
				
				float Opacity250 = (0.5 + (( 1.0 - _ShadowOpacity ) - 0.0) * (1.0 - 0.5) / (1.0 - 0.0));
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = lerpResult558.rgb;
